...
- Full rescan: manually inspect every WordPress post in the posts category
...
UA = "fivsevn-flux-sync/1.2 (+https://github.com/fivsevn/fivsevn-flux)"
...
def canonical_source(raw_html: str) -> str:
    ...
    so localizing images to ./assets/ does not create false positives.
    """
...
def cleanup_markdown(text: str) -> str:
    # Remove old WordPress stream footer lines: '.', tags:, comment:, count, ISO timestamp.
...
def maybe_write_package(item_dir: Path, fm: dict, body: str, force: bool = False) -> str:
    """Create/update/skip one package.

    Returns created|updated|skipped|manual.
    """
    ...
    # Hard force is intentionally only for local/manual use.
    # The full-rescan workflow does not pass --force, so sync: manual remains safe.
    if force:
        dump_package(item_dir, fm, body, force=True)
        return "updated"
...
def sync_rest(
    ...
    while True:
        ...
        try:
            ...
            # WordPress REST API returns 400 for pages beyond the last page.
...
def parse_live_html(
    ...
    # It cannot truly full-rescan older paginated WordPress content.
...
def main():
    ...
    if args.html_only:
        stats = parse_live_html(args.posts_url, args.limit, args.full_rescan, args.force, args.no_media)
    else:
        try:
            stats = sync_rest(site, args.limit, args.full_rescan, args.force, args.no_media)
